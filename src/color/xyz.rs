    ...
    F: Float,
{
    ...
    fn from(lab: &Lab<F>) -> Self {
        ...
        let x = F::from_f64(0.95046) * f(l2 + a2);
        let y = F::from_f64(1.0) * f(l2);
        let z = F::from_f64(1.08906) * f(l2 - b2);
        XYZ::new(x, y, z)
    }
}
...
mod tests {
    ...
    fn from_rgba_should_convert_to_xyz() {
        ...
        assert_eq!(
            XYZ::from(&white),
            XYZ::new(0.9504560232162476, 1.0, 1.088644027709961)
        );

        let red = Rgba::red();
        assert_eq!(XYZ::from(&red), XYZ::new(0.412391, 0.212639, 0.019331));

        let green = Rgba::green();
        assert_eq!(XYZ::from(&green), XYZ::new(0.357584, 0.715169, 0.119195));

        let blue = Rgba::blue();
        assert_eq!(XYZ::from(&blue), XYZ::new(0.180481, 0.072192, 0.950532));
    ...
    fn from_lab_should_convert_to_xyz() {
        ...
        assert_eq!(
            XYZ::from(&red),
            XYZ::new(0.412391, 0.212639, 0.01933100000000002)
        );

        let green = Lab::new(87.73553464128194, -86.18229362351477, 83.1866539998871);
        assert_eq!(
            XYZ::from(&green),
            XYZ::new(0.35758400000000007, 0.715169, 0.11919500000000006)
        );

        let blue = Lab::new(32.30080257229819, 79.1952752634909, -107.85544501392465);
        assert_eq!(
            XYZ::from(&blue),
            XYZ::new(0.18048099999999997, 0.072192, 0.9505320000000002)
        );
    }
}
